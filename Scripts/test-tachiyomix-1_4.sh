...
fixture_name="tachiyomix-extension-lib-1_4-fixture.jar"
fixture_path="$fixture_root/$fixture_name"
fixture_url="${TACHIYOMIAZ_EXTLIB_1_4_FIXTURE_URL:-${TACHIAZ_EXTLIB_1_4_FIXTURE_URL:-}}"
...
if [[ -n "${1:-}" ]]; then
    ...
    command -v curl >/dev/null || {
        echo "curl is required to download the extension-lib 1.4 fixture." >&2
        exit 1
    }
    mkdir -p "$fixture_root"
    if [[ ! -f "$fixture_path" ]]; then
        if [[ -z "$fixture_url" ]]; then
            echo "Pass the extension-lib 1.4 JAR path or set TACHIYOMIAZ_EXTLIB_1_4_FIXTURE_URL." >&2
            exit 1
        fi
...
if [[ "$actual_sha256" != "$expected_sha256" ]]; then
    echo "Extension-lib 1.4 fixture checksum mismatch." >&2
...
TACHIAZ_EXTLIB_1_4_JAR="$fixture_path" \
